...
def convert_to_voltage(buffer, block):
    for i in range(0, len(block)-1, 2):
        voltage = block[i] << 8 | block[i+1]
        buffer.append(voltage - (1650+20000))
...
def acquire_data_and_count(channel, buffer, stop_event):
    ...
    try:
        while not stop_event.is_set():
            sample_counter += 16
            block = bus.read_i2c_block_data(0x08, 0x20, 32)
            convert_to_voltage(buffer, block)
